 ...
 */
function removeLastOccurrences(str, times) {
  const lastIndex = str.lastIndexOf(times);

  if (lastIndex !== -1) {
    return str.slice(0, lastIndex) + str.slice(lastIndex + times.length);
  }
...
function reverseWords(str) {
  const a = [];
  str.split(' ').forEach((e) => a.push(e.split('').reverse().join('')));
  return a.join(' ');
}
...
function invertCase(str) {
  return str.replace(/\w/g, (e) =>
    String(e.match(/[A-Z]/g)) === e ? e.toLowerCase() : e.toUpperCase()
  );
}
...
function extractNameFromTemplate(value) {
  return value.replace('Hello, ', '').replace(/!/g, '');
}
